...
if (type === "package") {
	...
	execSync(
		`deno run npm:turbo gen workspace --name ${name} --copy template-library --type package --destination packages/${name}`,
		{ stdio: "inherit" },
	);
	...
	const cmd = `find ./packages/${name} -type f -exec sed -i '' 's/template-library/${name}/g' {} +`;
...
if (type === "app") {
	...
	execSync(
		`deno run npm:turbo gen workspace --name ${name} --copy template-app --type app --destination apps/${name}`,
		{ stdio: "inherit" },
	);
	...
	const cmd = `find ./apps/${name} -type f -exec sed -i '' 's/template-app/${name}/g' {} +`;
	execSync(cmd, { stdio: "inherit" });
